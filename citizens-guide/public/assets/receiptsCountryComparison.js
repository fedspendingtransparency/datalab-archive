...
function addText() {
  var props = this,
      text = containers.data.selectAll('text' + props.class).data(data).enter().append('text').classed(props.class, true).text(function (d) {
    return (0, _utils.simplifyNumber)(d[props.data]);
...
function drawBars(which) {
  var props = map[which],
      ...
      bars = containers.data.selectAll('rect.' + props.class).data(data).enter().append('rect').classed(props.class, true).attr('width', scales.x(0)).attr('height', dimensions.barHeight).attr('x', 0).attr('y', function (d, i) {
...
function drawXAxis() {
  var xAxis = d3.axisBottom(scales.x) // .tickValues(yTicks)
  ...
  var axisGroup = containers.data.append('g').attr('transform', (0, _utils.translator)(0, dimensions.totalHeight)).call(xAxis);
  axisGroup.selectAll('.tick line').attr('y1', 0 - dimensions.totalHeight).attr('stroke', '#eee');
  axisGroup.selectAll('.domain').remove();
}

function placeCountryLabels() {
  containers.country.selectAll('text').data(data).enter().append('text').text(function (d) {
    return d.country;
  }).attr('y', function (d, i) {
...
function placeGdpFigures() {
  containers.gdp.selectAll('text').data(data).enter().append('text').text(function (d) {
...
function chartInit(_data, container) {
  data = _data;
...
var masterData = (0, _data.prepareData)(_incomeCountryComparison.default),
    svg = (0, _utils.establishContainer)(600),
...
(0, _chart.chartInit)(filterData(), svg);
...
function buildCountryBox() {
  containers.country.append('rect').attr('width', dimensions.countryColumnWidth).attr('height', dimensions.totalHeight).attr('fill', 'white').attr('stroke', '#eee').style('filter', 'url(#drop-shadow)');
}

function buildGdpBox() {
  containers.gdp.append('rect').attr('width', dimensions.gdpColumnWidth).attr('height', dimensions.totalHeight).attr('fill', 'white').attr('stroke', '#eee').style('filter', 'url(#drop-shadow)');
}

function placeHorizontalStripes(dataLength) {
  var stripes = containers.chart.append('g').attr('transform', (0, _utils.translator)(0, dimensions.header));
  stripes.selectAll('line').data(d3.range(dataLength + 1)).enter().append('line').attr('x1', 0).attr('y1', function (d) {
...
function placeMask() {
  containers.legends.append('rect').attr('width', 1200).attr('height', dimensions.header).attr('stroke', 'none').attr('fill', 'white');
}
...
var activeCountries, parentDiv, listDiv, trigger;
...
function listActiveCountries() {
  activeCountries = _chart.countryList.get();
  listDiv.append('ul').selectAll('li').data(activeCountries).enter().append('li').each(function (d) {
    this.innerText = d.country;
  });
}

function getAvailableCountries() {
  var activeList = activeCountries.map(function (r) {
    return r.country;
  });
...
function listAvailableCountries() {
  var list = getAvailableCountries(),
      max = 10;
  var more, remainder;
  ...
  listDiv.selectAll('div.available').data(list).enter().append('div').classed('available', true).each(function (d) {
    this.innerText = d;
  });

  if (more) {
    listDiv.append('div').classed('see-more', true).each(function () {
...
function establishContainer(height) {
  ...
  if (svg.size() === 0) {
    return viz.append('svg').attr('shape-rendering', 'geometricPrecision').attr('height', height).attr('width', '1200px');
  } else {
    return svg;
  }
}
