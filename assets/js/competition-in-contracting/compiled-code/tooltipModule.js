...
var tooltipModule = function tooltipModule() {
  function draw(title, information) {
    ...
    d3.select("#tooltip").html(toolTipHtml(title, information)).style("left", d3.event.pageX + "px").style("top", d3.event.pageY + "px");
  ...
  function remove() {
    d3.select("#tooltip").transition().duration(500).style("opacity", 0).style("pointer-events", "none");
  }
