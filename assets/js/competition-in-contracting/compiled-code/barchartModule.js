...
var barchartModule = function barchartModule() {
  ...
  function draw(data, settings, helpers) {
    ...
    var x = void 0;
    if (settings.xAxisScale === "quantity") {
      x = d3.scale.linear().domain([0, d3.max(sortedData, function (d) {
    ...
    var keys = settings.xAxisScale === "quantity" ? ["competed", "notCompeted"] : ["percentCompeted", "percentNotCompeted"];
    ...
    var ticklength = 525;
    g.append("g").attr("class", "axis axis--x").attr("transform", "translate(0," + (height - ticklength) + ")").call(d3.svg.axis().scale(x).orient("bottom")
    // .ticks(10)
    .tickFormat(d3.format(tickFormat)).tickSize(ticklength)).attr("class", "xTick").selectAll("text").style("text-anchor", "end").style("font-size", "12px").attr("transform", "rotate(-35) translate(-295,-95)").attr("dx", "-.8em")
    // .attr("dy", ".35em")
    .attr("pointer-events", "none");

    // y axis
    g.append("g").attr("transform", "translate(-12,0)").attr("class", "axis axis--y").call(d3.svg.axis().orient("left").scale(y)).selectAll(".tick").attr("class", "yTick").selectAll("text").style("font-size", "12px");

    // y axis checkboxes
    g.append("g").attr("class", "axis axis--y").attr("y", 6).attr("dy", "0.71em").append("foreignObject").attr("transform", "translate(-20,0)").attr("width", 10).attr("height", 1000).append("xhtml:body").append("form").attr("id", "yAxisCheckboxes").selectAll(".yAxisCheckbox").data(sortedData).enter().append("div").style("height", "21px").style("background", "white").append("input").attr("type", "checkbox").attr("checked", function (d) {
      return d.displayed ? true : null;
    }).attr("class", ".yAxisCheckbox").attr("id", function (d) {
    ...
    var stackedDataset = d3.layout.stack()(keys.map(function (key) {
    ...
    // bars
    g.append("g").selectAll(".barGroup").data(stackedDataset).enter().append("g").attr("fill", function (d, i) {
      return z(keys[i]);
    }).attr("class", "barGroup").selectAll(".bar").data(function (d) {
      return d;
    ...
    function handleMouseOver(d) {
      tooltipModuleDraw(d.data.name, {
        Competed: settings.xAxisUnit === "dollars" ? formatNumber("dollars", d.data.competed) : formatNumber("actions", d.data.competed),
        "Not Competed": settings.xAxisUnit === "dollars" ? formatNumber("dollars", d.data.notCompeted) : formatNumber("actions", d.data.notCompeted),
